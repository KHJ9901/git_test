...
import com.plant.service.DicService;
...
public class RestControllerSample {
	...
	public Dictionary getBoard() {  
		return ds.DicDetail("20");
	...
	public Map<String, Dictionary> getMap() {
		...
		map.put("TwentyOne", ds.DicDetail("1"));
		map.put("TwentyTwo", ds.DicDetail("2"));
		return map;
	}
	...
	public ResponseEntity<List<Dictionary>> check(Integer page, Integer record) {
		Criteria cri = new Criteria(page, record);
		
		List<Dictionary> boardList = ds.list(new Criteria(page, record));
		
		ResponseEntity<List<Dictionary>> result = null;
	...
	public List<Dictionary> getSearchBoard(
	...
	@PostMapping("member")
	public Plantmember convert(@RequestBody Plantmember member) {
